...
-- Test insert permission (without user_id column)
INSERT INTO playlist_items (id, playlist_id, position, audio_file_id, added_at) 
VALUES (gen_random_uuid(), (SELECT id FROM playlists LIMIT 1), 1, (SELECT id FROM audio_files LIMIT 1), NOW());

-- Clean up test data
DELETE FROM playlist_items WHERE position = 1;
